...
def mongo_query(collection:object, search:str=None):
    if search==None:
       return collection.find({}, {"_id":0}) 
    else:
        return collection.find({"name":{"$regex":search,"$options":"i"}}, {"_id":0})

def mongo_card(collection:object):
    return collection.find({}, {"_id":0,"img_url":1, "name":1,"receipt_term":1,"use_term":1,"camp_url":1,"row":1}) 

def mongo_card_one(collection:object, row):
    return collection.find_one({"row":row}, {"_id":0,"img_url":1, "name":1,"receipt_term":1,"use_term":1,"camp_url":1,"row":1}) 


def mongo_find_one_name(collection:object, name:str):
    return collection.find_one({"name":{"$regex":name,"$options":"i"}},{"_id":0})

def mongo_find_one_row(collection:object, row):
    return collection.find_one({"row":row}, {"_id":0}) 
